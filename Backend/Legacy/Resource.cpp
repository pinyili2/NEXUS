...
void *Resource::get_stream(StreamType stream_type) const {
  ...
  if (!sycl_queues_ && !cuda_streams_) {
    initialize_streams();
  }

#ifdef USE_SYCL
  if (type == ResourceType::SYCL) {
    size_t idx = sycl_next_queue_.fetch_add(1) % 8;
...
#ifdef USE_CUDA
  if (type == ResourceType::CUDA) {
    size_t idx = cuda_next_stream_.fetch_add(1) % 8;
    return reinterpret_cast<void *>((*cuda_streams_)[idx].get());
  }
#endif
