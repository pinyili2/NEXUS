...
#ifdef USE_NCCL
    if (res.getType() == ResourceType::CUDA) {
...
public:
  ...
  void benchmark_backends() {
    ...
    MPI::Manager::instance().allReduce(test_buffer, test_buffer.size(),
                                       resource_);
    ...
    if (resource_.type == ResourceType::CUDA) {
      ...
      NCCL::Manager::instance().allReduce(test_buffer, test_buffer.size(),
                                          resource_);
      ...
      LOGINFO("MPI: {}ms, NCCL: {}ms",
              std::chrono::duration_cast<std::chrono::milliseconds>(mpi_time)
                  .count(),
              std::chrono::duration_cast<std::chrono::milliseconds>(nccl_time)
                  .count());
    }
#endif
  };
  template <typename T> Event allReduce(DeviceBuffer<T> &buffer, idx_t count) {
...
#ifdef USE_NCCL
    case Backend::NCCL:
      return NCCL::Manager::instance().broadcast(buffer, count, root,
                                                 resource_);
#endif
    default:
...
private:
  struct DeviceNode {
    ...
    void *get_queue(int device_id) const { return primary_stream; }
...
public:
  ...
  explicit DeviceMesh(std::vector<Resource> resources) : resources_(resources) {
    ...
    for (const auto &res : resources_) {
      ...
      if (res.type() == ResourceType::SYCL) {
        // Use Resource's stream management for SYCL
        node.primary_stream = res.get_stream();
        queues_[res.id()] = node.primary_stream;
      }
#endif

#ifdef USE_METAL
      if (res.getType() == ResourceType::METAL) {
    ...
    for (size_t i = 0; i < resources_.size(); ++i) {
      ...
      for (size_t j = 0; j < resources_.size(); ++j) {
        if (i != j && resources_[j].type == ResourceType::CUDA) {
          int can_access;
          cudaDeviceCanAccessPeer(&can_access, resources_[i].id,
                                  resources_[j].id);
          if (can_access) {
  ...
  // Getter methods
  size_t device_count() const { return nodes_.size(); }
  ...
  const std::vector<Resource> &get_resources() const { return resources_; }

  bool is_peer_access_enabled() const { return peer_access_enabled_; }
};
} // namespace ARBD
#endif
