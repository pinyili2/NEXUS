...
private:
  static inline std::unordered_map<std::string, std::shared_ptr<CUDAEvent>>
      active_events_;
};
...
public:
  ...
  public:
    ScopedRange(const std::string &name) : name_(name) { start_range(name_); }
    ~ScopedRange() { end_range(name_); }
  };

private:
  static inline std::unordered_map<std::string, std::shared_ptr<SYCLEvent>>
      active_events_;
...
// Enable GPU timeline capture if available
  ...
  static std::shared_ptr<METALEvent> create_event(const std::string &name,
  ...
  static void profile_memory_usage(void *device_ptr) {
    ...
    LOGINFO("Metal Device Memory: Recommended working set {} MB, Unified "
            "Memory: {}",
...
private:
  static inline std::unordered_map<std::string, std::shared_ptr<METALEvent>>
      active_events_;
  static inline bool profiling_enabled_ = false;
};
 ...
 * @brief Unified interface for profiling across all supported backends (CUDA,
 * SYCL, Metal).
 *
 * The ProfileManager class provides a backend-agnostic API for initializing,
 * finalizing, and managing profiling events, memory usage, and markers. It
 * coordinates profiling across the enabled backend (CUDA, SYCL, or Metal), and
 * collects timing and memory usage data for performance analysis.
 *
 * Usage:
 *   - Call ProfileManager::init() at the start of profiling.
 *   - Use PROFILE_RANGE, PROFILE_MARK, and PROFILE_MEMORY macros to annotate
 * code regions.
 *   - Call ProfileManager::finalize() to end profiling and optionally save
 * results.
 *
 * Features:
 *   - Supports enabling/disabling timing, memory tracking, and kernel
 * profiling.
 ...
 *   - Backend-specific profiling is handled transparently based on compile-time
 * flags.
 *
 * Note:
...
public:
  static void init(const ProfilingConfig &config = {}) {
    ...
    if (!config_.enable_backend_markers)
      return;
  ...
  static void start_range(const std::string &name, ResourceType backend) {
    if (!initialized_ || !config_.enable_backend_markers)
      return;
    ...
    if (it != active_ranges_.end()) {
      ...
      if (events_.size() > config_.max_events) {
        events_.erase(events_.begin(),
                      events_.begin() + (events_.size() - config_.max_events));
      }
    }
  ...
  static void profile_memory_usage(ResourceType backend,
                                   void *backend_specific_ptr = nullptr) {
    ...
    case ResourceType::SYCL:
      ...
      if (backend_specific_ptr) {
        SYCLProfiler::profile_memory_usage(
            *static_cast<sycl::queue *>(backend_specific_ptr));
      }
#endif
      break;
    case ResourceType::METAL:
  ...
  public:
    ScopedTimer(const std::string &name, void *queue = nullptr)
        : timer_(name, queue) {}
  ...
  public:
    ...
    ~ScopedRange() { end_range(name_, backend_); }
  };
  ...
  static void print_summary() {
    ...
    for (const auto &[backend, times] : backend_times) {
      ...
      LOGINFO("%zu events, %.3f ms total, %.3f ms avg, %.3f/%.3f ms min/max",
              times.size(), total_time, total_time / times.size(), min_time,
              max_time);
    }
  }
...
private:
  ...
  static inline std::unordered_map<std::string, std::shared_ptr<ProfileEvent>>
      active_ranges_;
};
