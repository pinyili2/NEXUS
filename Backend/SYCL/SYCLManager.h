...
public:
  ...
  // **MODIFIED**: Explicitly create a single-device context to prevent
  // ambiguity. This guarantees that the queue is not considered "multi-device"
  // by the runtime.
  ...
  [[nodiscard]] const sycl::device &get_device() const { return device_; }
