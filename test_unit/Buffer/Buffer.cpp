...
TEST_CASE_METHOD(ProductionBufferTestFixture,
	...
	SECTION("Empty buffer construction") {
		...
		REQUIRE(empty_buffer.resource().is_valid());
	}
}
...
TEST_CASE_METHOD(ProductionBufferTestFixture, "Buffer Memory Operations", "[Buffer][memory]") {
	...
	SECTION("Error handling - oversized copy") {
		...
		// The implementation automatically resizes the buffer to match the host data size
		// So we test that the buffer is resized correctly instead of expecting an exception
...
TEST_CASE_METHOD(ProductionBufferTestFixture, "Buffer Edge Cases", "[Buffer][edge_cases]") {
	...
	SECTION("Copy operations with zero-size buffers") {
		...
		// Copy from empty vector to empty buffer should also throw since device_ptr_ is null
...
TEST_CASE_METHOD(ProductionBufferTestFixture,
	...
	SECTION("Resource Local() detection") {
		...
		REQUIRE(local_resource.is_valid());
...
TEST_CASE_METHOD(ProductionBufferTestFixture,
	...
	SECTION("Exception safety guarantee") {
		...
		try {
			...
			// The implementation automatically resizes the buffer, so no exception is thrown
