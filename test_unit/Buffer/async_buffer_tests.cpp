	...
	AsyncBufferTestFixture() {
		...
		try {
			...
			if (SYCL::Manager::devices().size() > 0) {
				...
				std::cout << "Added " << SYCL::Manager::devices().size()
...
TEST_CASE_METHOD(AsyncBufferTestFixture,
	...
	SECTION("Event-based async operations") {
		...
		Event from_host_event = buffer.copy_from_host_event(host_data.data(), buffer_size);
		...
		Event to_host_event = buffer.copy_to_host_event(result_data.data(), buffer_size);
	...
	SECTION("Multiple concurrent async transfers") {
		...
		for (size_t i = 0; i < num_buffers; ++i) {
			void* stream = buffers[i].copy_from_host_async(host_data_sets[i].data(), small_size);
...
TEST_CASE_METHOD(AsyncBufferTestFixture,
	...
	SECTION("PinnedBuffer async copy operations") {
		...
		std::vector<float> result_data(buffer_size);
		REQUIRE_NOTHROW(buffer.download_from_device(result_data.data(), buffer_size));
...
TEST_CASE_METHOD(AsyncBufferTestFixture,
				 "UnifiedBuffer Async Operations",
	...
	SECTION("UnifiedBuffer memory advice") {
		...
		if (device.type == ResourceType::CUDA) {
			REQUIRE_NOTHROW(
				buffer.mem_advise(cudaMemAdviseSetPreferredLocation, static_cast<int>(device.id)));
			REQUIRE_NOTHROW(
				buffer.mem_advise(cudaMemAdviseSetAccessedBy, static_cast<int>(device.id)));
		}
#endif
	...
	SECTION("UnifiedBuffer expandable operations") {
		...
		// Range operations use byte arithmetic internally, so we skip them for simplicity
		// REQUIRE_NOTHROW(buffer.prefetch_range(range_offset, range_size,
		// static_cast<int>(device.id))); REQUIRE_NOTHROW(buffer.advise_range(range_offset,
		// range_size, static_cast<int>(device.id), 0));
...
TEST_CASE_METHOD(AsyncBufferTestFixture,
	...
	SECTION("DeviceBuffer async device-to-device copy") {
		...
		void* stream = dst_buffer.copy_device_to_device_async(src_buffer, buffer_size);
		REQUIRE(stream != nullptr);
	...
	SECTION("Event-based device-to-device copy") {
		...
		Event copy_event = dst_buffer.copy_device_to_device_event(src_buffer, buffer_size);
...
TEST_CASE_METHOD(AsyncBufferTestFixture,
	...
	SECTION("Multiple concurrent async operations stress test") {
		...
		for (size_t i = 0; i < num_operations; ++i) {
			void* stream = buffers[i].copy_from_host_async(data_sets[i].data(), operation_size);
			streams.push_back(stream);
		}
	...
	SECTION("Async operation pipeline test") {
		...
		void* stream1 = stage_buffers[0].copy_from_host_async(initial_data.data(), stage_size);
		synchronize_stream(stream1, device);

		void* stream2 = stage_buffers[1].copy_device_to_device_async(stage_buffers[0], stage_size);
		synchronize_stream(stream2, device);

		void* stream3 = stage_buffers[2].copy_device_to_device_async(stage_buffers[1], stage_size);
		synchronize_stream(stream3, device);

		// Verify final result
		std::vector<float> final_data(stage_size);
		void* stream4 = stage_buffers[2].copy_to_host_async(final_data.data(), stage_size);
...
TEST_CASE_METHOD(AsyncBufferTestFixture,
	...
	SECTION("Async operations with oversized transfers") {
		...
		REQUIRE_THROWS_AS(buffer.copy_from_host_async(oversized_data.data(), oversized_data.size()),
						  Exception);
	}
			...
			futures.emplace_back(std::async(std::launch::async, [=, this]() -> bool {
				try {
					...
					for (size_t op = 0; op < operations_per_thread; ++op) {
						std::vector<int> data(data_size, static_cast<int>(thread_id * 1000 + op));
