...
TEST_CASE("Resource Abstraction", "[backend][resource]") {
	...
	SECTION("Default resource construction") {
		...
		REQUIRE(default_resource.is_valid());
	...
	SECTION("Resource construction with ID") {
		...
		REQUIRE(resource_0.is_valid());
		REQUIRE(resource_1.is_valid());
...
TEST_CASE("KernelConfig Abstraction", "[backend][kernel_config]") {
	...
	SECTION("Problem size calculation") {
		...
		size_t total_threads = (config.block_size.x * config.block_size.y * config.block_size.z) *
...
TEST_CASE("Event Abstraction", "[backend][event]") {
	...
	SECTION("EventList container operations") {
		...
		// Note: EventList::add() only adds valid events, so empty() result depends on
		// implementation
	}
...
TEST_CASE("Advanced Backend Features", "[backend][advanced]") {
	...
	SECTION("Backend Configuration Edge Cases") {
		...
		REQUIRE(invalid_resource.is_valid()); // CPU resources are always valid
	}
...
TEST_CASE("Backend Integration", "[backend][integration]") {
	...
	SECTION("Multi-buffer kernel execution") {
		if (g_backend_available) {
			try {
				...
				config1.sync = true;
				auto event1 = launch_kernel(resource, config1, scale_kernel, input_buf, temp_buf);
				...
				auto event2 = launch_kernel(resource, config2, add_kernel, temp_buf, output_buf);
				...
				for (size_t i = 0; i < std::min(n, size_t(5)); ++i) {
					...
					INFO("Element " << i << ": input=" << input_data[i] << ", result=" << result[i]
									<< ", expected=" << expected);
				}
	...
	SECTION("Performance characteristics") {
		if (g_backend_available) {
			...
			for (int i = 0; i < num_launches; ++i) {
				auto event = launch_kernel(resource, config, simple_kernel, input_buf, output_buf);
				event.wait();
			}
			...
			// Performance should be reasonable (less than 1 second total for 10 launches)
			INFO("Total time for " << num_launches << " kernel launches: " << duration.count()
								   << "ms");
	...
	SECTION("Multi-Device Buffer Transfers") {
		...
		INFO("✓ Device 1 data verified: " << verify_device1[0] << ", " << verify_device1[1] << ", "
										  << verify_device1[2] << "...");
		...
		INFO("✓ Device 2 data verified: " << verify_device2[0] << ", " << verify_device2[1] << ", "
										  << verify_device2[2] << "...");
		...
		auto end = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
		INFO("Device-to-device transfer time for " << n << " floats: " << duration.count()
												   << " μs");
	}

	SECTION("2D Matrix Pattern with Unified Buffers") {
		INFO("Testing 2D matrix with odd number pattern (1,3,5,7,9...) using Unified buffers");
		...
		INFO("Creating " << rows << "x" << cols << " matrix with " << total_size << " elements");
		...
		try {
			...
			unified_buf.mem_advise(0, device_resources[0].id); // Set preferred location to device 0
			...
			REQUIRE(std::abs(get_matrix_element(0, 0) - 1.0f) < 1e-6f); // First element = 1
			REQUIRE(std::abs(get_matrix_element(0, 1) - 3.0f) < 1e-6f); // Second element = 3
			REQUIRE(std::abs(get_matrix_element(1, 0) - (cols * 2 + 1)) <
					1e-6f); // Start of second row
			REQUIRE(std::abs(get_matrix_element(rows - 1, cols - 1) - ((total_size - 1) * 2 + 1)) <
					1e-6f); // Last element
			...
			if (device_resources.size() > 1) {
				...
				unified_buf2.mem_advise(0, device_resources[1].id); // Set preferred location
				...
				// Test direct memory access if possible (unified memory should be accessible)
				...
				for (size_t row = 0; row < num_test_rows; ++row) {
					unified_buf2.advise_range(row * matrix_row_size,
											  matrix_row_size,
			...
			REQUIRE(std::abs(verify_device[total_size - 1] - ((total_size - 1) * 2 + 1)) < 1e-6f);
