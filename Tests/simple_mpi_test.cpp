...
// Simple MPI test without SYCL dependencies
TEST_CASE("Simple MPI Test", "[mpi][simple]") {
#ifdef USE_MPI
  // Initialize MPI
  ARBD::MPI::Manager::instance().ensure_initialized();
  ...
  SECTION("6 Process Communication") {
    // Each process sends its rank * 10 (same as first test)
    int send_data = rank * 10;
    int recv_data = 0;
  ...
  // Finalize MPI
  ARBD::MPI::Manager::instance().ensure_finalized();
