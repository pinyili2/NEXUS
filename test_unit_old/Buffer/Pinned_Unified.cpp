    ...
    if (initialized_.compare_exchange_strong(expected, true)) {
      ...
      try {
        ...
        if (!CUDA::Manager::devices().empty()) {
          CUDA::Manager::use(0);
          std::cout << "Initialized CUDA with "
                    << CUDA::Manager::devices().size() << " device(s)"
                    << std::endl;
        }
#endif
        ...
        if (!SYCL::Manager::devices().empty()) {
          SYCL::Manager::use(0);
          std::cout << "Initialized SYCL with "
                    << SYCL::Manager::devices().size() << " device(s)"
                    << std::endl;
        }
#endif
...
TEST_CASE_METHOD(BackendInitFixture, "Buffer Edge Cases", "[edge_cases]") {
  ...
  SECTION("Comprehensive buffer resize testing") {
    ...
    for (size_t new_size : sizes) {
      ...
      // For UnifiedBuffer, use the base class resize method
      static_cast<Buffer<float, UnifiedPolicy> &>(multi_unified)
          .resize(new_size);
  ...
  SECTION("Buffer resize edge cases and error handling") {
    ...
    // These operations might fail due to memory constraints, but shouldn't
    // crash
    REQUIRE_NOTHROW(large_pinned.resize(very_large_size));
    // For UnifiedBuffer, use the base class resize method
    REQUIRE_NOTHROW(static_cast<Buffer<float, UnifiedPolicy> &>(large_unified)
                        .resize(very_large_size));
    ...
    // Try to resize with a different resource (this might be the same on
    // single-device systems)
    Resource alt_resource = Resource::Local();
    REQUIRE_NOTHROW(resource_pinned.resize(200, alt_resource));
    // For UnifiedBuffer, use the base class resize method
    REQUIRE_NOTHROW(
        static_cast<Buffer<float, UnifiedPolicy> &>(resource_unified)
            .resize(200, alt_resource));
    ...
    for (int i = 0; i < 20; ++i) {
      ...
      // For UnifiedBuffer, use the base class resize method
      REQUIRE_NOTHROW(static_cast<Buffer<float, UnifiedPolicy> &>(rapid_unified)
                          .resize(new_size));
